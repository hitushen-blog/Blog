  ...
  <div>
    <el-form :model="form" :rules="formRules" ref="formRef" label-position="top">
      ...
      <el-form-item label="文章描述" prop="description">
        <mavon-editor ref="md_one" v-model="form.description" @imgAdd="imgAdd('md_one',arguments)"/>
      </el-form-item>

      <el-form-item label="文章正文" prop="content">
        <mavon-editor ref="md_two" v-model="form.content" @imgAdd="imgAdd('md_two',arguments)"/>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    // 上传地址是shenyunpeng的minio
    imgAdd(site, [pos, file]) {
      var formData = new FormData()
      formData.append('file', file)
      axios
          .post('https://csapi.hitushen.top/oss/upload', formData, {
            headers: {
              'Content-Type': 'multipart/form-data'
            }
          })
          .then((res) => {
            this.$refs[site].$img2Url(pos, res.data.data)
          })
          .catch((error) => {
            this.msgError(error)
          });
